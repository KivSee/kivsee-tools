...
def stop():
    ...
    response = requests.post(
        f"{config.raspberry_pi_addr}:{config.trigger_service_port}/stop")
...
def play_song(trigger_name, start_offset_ms: int = 0):
    ...
    response = requests.post(
        f"{config.raspberry_pi_addr}:{config.trigger_service_port}/song/{trigger_name}/play", json=trigger_song_options)
    logger.info(
        f"Triggering song {trigger_name} - status code ({response.status_code})")
    return response
...
def play_soundless_animation(trigger_name, start_offset_ms: int = 0):
    ...
    response = requests.post(
        f"{config.raspberry_pi_addr}:{config.trigger_service_port}/trigger/{trigger_name}", json=trigger_animation_options)
    logger.info(
        f"Triggering animation {trigger_name} - status code ({response.status_code})")
    return response
...
def store_sequence_all(trigger_name, per_thing_config):
    logger.debug(
        f"Storing sequence of multiple things, amount {len(per_thing_config)}")
    response = requests.put(
...
def get_all_segments():
    ...
    response = requests.get(
        f"{config.raspberry_pi_addr}:{config.object_service_port}/thing")
    logger.info(
        f"Get all segments - status code ({response.status_code})")
    return response.content


def get_trigger_config(trigger_name: str):
    logger.info(f"Getting trigger config")
    response = requests.get(
        f"{config.raspberry_pi_addr}:{config.sequence_service_port}/triggers/{trigger_name}/config")
    if response.ok:
        return response.json()
    else:
        logger.error("Failed to get trigger config. status code: %d", response.status_code)
        return None
