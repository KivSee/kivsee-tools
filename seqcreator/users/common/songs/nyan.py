...
class Nyan(Song):
    ...
    # 2 colors on segments

    def set_random_coloring(self):
        coloring_func = random.choice([self.segments_color])
        # coloring_func = random.choice([self.uniform_color, self.rainbow_color, self.segments_color])
        coloring_func()
