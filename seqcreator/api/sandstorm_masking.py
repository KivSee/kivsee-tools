...
def brightness_sin(elements, options):
    ...
    masking.brightness_sin(0.1, 0.5)
...
def alternate_sin(elements, options):
    ...
    elements.set(all_elements_a2)
    get_effects().add_effect(BrightnessEffect(sin_function(0.0, 0.4, 0.25, 1)))
...
def snake_grow_shrink(elements, options):
    ...
    get_effects().add_effect(SnakeEffect(const_function(0.5), sin_function(0.0, 0.5, 0.0, 1.0)))

def snake_step(elements, options):
    timing.cycle(4)
    elements.set(all_elements_single_sym)
    get_effects().add_effect(SnakeEffect(steps_function(16, 0.125, 0.125), const_function(8.0)))
